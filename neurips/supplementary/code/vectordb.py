...
async def create_memory_collection(): 
    if not (await client.collection_exists(COLLECTION_NAME)): 
        ...
        print("Collection created with user_handle, timestamp and radix_paths indices")
    else:
...
async def search_memories(
    ...
    print(f"📡 [QDRANT-QUERY] {COLLECTION_NAME} | user_id: {user_id} | handle: {user_handle} | limit: {limit}")
...
def convert_retrieved_records(point) -> RetrievedMemory:
    ...
    return RetrievedMemory(
        ...
        memory_text=payload.get("memory_text") or payload.get("memory_texts", ""),
